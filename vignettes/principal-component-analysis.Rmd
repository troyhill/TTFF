...
por <- mergeData(rainfall = rainDat, PET = PETDat, 
                 ...
                 # instead of using Monday as the first day of the week,
                 # SFWMD uses Friday. Arbitrary, but let's follow that.
...
# testDat$predFlow.sqrt <- sqrt(testDat$predFlow)
# 
# plot(predFlow.sqrt, sumFlow,  test[test$sumFlow > 0, ])
...
newDat$flowPredictions <- estimateFlow(newData = newDat, PCA_output = pca1, 
                                periodOfRecord = por)

summary(lm1 <- stats::lm(sumFlow ~ I(sqrt(flowPredictions)), data = newDat[newDat$sumFlow > 0, ]))
